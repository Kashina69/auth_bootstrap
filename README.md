...
- **FIXED (2026-09-14): `isSystem` permissions were deletable.** The service derived "baseline" from system-role grants, but plan §5 declares 7 baseline permissions while system roles grant only 5 — so `update:Post`, `delete:Post`, `read:User` were deletable via `DELETE /rbac-admin/permissions/:id`. Fixed by adding the real `is_system` column (**a migration was required** — `permissions` never had it; an earlier orchestrator claim that it already existed was wrong), exposing `Permission.isSystem`, and adding the two missing contract methods (`UserRepository.findUserIdsByRole`, `RoleRepository.detachPermissions`) that had also blocked plan §5.5's `invalidate()` fan-out and Phase 9's detach CRUD.
- **Seed backfill caveat:** the seed's `findOrCreatePermission` never updates an existing row, so any environment that ran the old seed keeps `is_system = false` on those permission rows until they are recreated — there is no permission-update method in the contract to backfill with.
- **`deletePermission` does not fan out `invalidate()`** (only the three methods plan §5.5 names do); finding a permission's holders would need a further contract method.
